...
export class Match{
 ...
 this.pickups=this.pickups.filter(p=>this.config.mode!=='instagib'&&(modeWeapon(this.config)===null||p.kind==='health'||p.kind==='armor'));
  ...
  vehicleCollision(next,vehicle){const radius=vehicleRadius(vehicle),bounds=boundsOf(this.arena),floor=floorAt(next.x,next.z,this.arena);if(floor===null||next.x<bounds.minX+radius||next.x>bounds.maxX-radius||next.z<bounds.minZ+radius||next.z>bounds.maxZ-radius)return false;if(obstructed(next.x,floor,next.z,radius,this.arena))return false;return {...next,y:floor};}
  ...
  refreshPowerups(a){a.speedMultiplier=1;a.damageMultiplier=1;a.cooldownMultiplier=1;a.temporaryShield=0;for(const id of Object.keys(a.powerups)){const p=POWERUPS.find(x=>x.id===id);if(!p||a.powerups[id]<=0){delete a.powerups[id];continue;}const e=p.effect;if(e.speedMultiplier)a.speedMultiplier=Math.max(a.speedMultiplier,e.speedMultiplier);if(e.damageMultiplier)a.damageMultiplier=Math.max(a.damageMultiplier,e.damageMultiplier);if(e.cooldownMultiplier)a.cooldownMultiplier=Math.min(a.cooldownMultiplier,e.cooldownMultiplier);if(e.armor)a.temporaryShield=Math.max(a.temporaryShield,e.armor);}}
    botInput(a,dt){const b=a.bot,hints=harnessBotHints(a.harness)||{range:[6,16],retreatHealth:.4,power:'hurt'},operator=operatorProfile(a.character);b.think-=dt;b.memory=Math.max(0,b.memory-dt);b.reaction=Math.max(0,b.reaction-dt);if(a.traversalFlight)return {};const ride=this.vehicles.find(vehicle=>vehicleCanEnter(vehicle,{...a,vehicle:a.vehicleId??null})&&Math.hypot(a.x-vehicle.position.x,a.z-vehicle.position.z)<2.4);if(ride&&!this.flagCarrier(a))return {interact:true};
  ...
  const supply=this.pickups.filter(p=>!p.wait&&this.useful(a,p)&&(walkEdge(a,p,this.arena)||path(a,p,this.nav,this.edges).length>1)).sort((c,d)=>(dist(a,c)-(c.kind==='health'&&a.health<a.maxHealth*.55?20:0))-(dist(a,d)-(d.kind==='health'&&a.health<a.maxHealth*.55?20:0)))[0];
    ...
    if(this.config.mode==='ctf'){const carrying=Object.values(this.flags).some(f=>f.carrier===a.id),enemy=this.flags[1-a.team],own=this.flags[a.team];if(carrying){b.state='flag-return';b.destination={x:own.x,y:0,z:own.z};}else if(needs){b.state='seek';b.destination=objectiveMode?needs:supply;}else if(enemy.state!=='carried'){b.state='flag-attack';b.destination={x:enemy.x,y:0,z:enemy.z};}else{b.state='flag-defend';b.destination={x:own.x,y:0,z:own.z};}}else if(needs){b.state='seek';b.destination=objectiveMode?needs:supply;}else if((this.config.mode==='koth'||this.config.mode==='domination')&&this.objectiveState){const zone=this.objectiveState.zones.filter(z=>z.owner!==a.team||z.captureTeam===a.team).sort((x,y)=>dist(a,x)-dist(a,y))[0]||this.objectiveState.zones[0];b.state='objective';b.destination={x:zone.x,y:floorAt(zone.x,zone.z,this.arena),z:zone.z};}else if(target){b.state='engage';b.destination={x:target.x,y:target.y,z:target.z};}else if(b.memory){b.state='pursue';b.destination=b.seen;}else{b.state='roam';if(!b.destination||dist(a,b.destination)<1.5)b.destination=this.nav[Math.floor(this.random()*this.nav.length)];}
