...
export const WEAPON_FINISHES=Object.freeze([
 finish('finish-ion','Ion Finish','A charged cyan finish that hums along the barrel.',4,'#22d3ee','#0e7490','#a5f3fc','#67e8f9'),
 finish('finish-ember','Ember Finish','A hot orange finish glowing with banked coals.',10,'#ff7a18','#7c2d06','#ffc48a','#ff9d4d'),
 finish('finish-void','Void Finish','A deep violet finish that swallows the light.',18,'#8b5cf6','#4c1d95','#c4b5fd','#a78bfa'),
 finish('finish-toxic','Toxic Finish','An acid green finish that drips with menace.',6,'#7cff3a','#2f6b18','#d9ff9e','#a3ff5e'),
 finish('finish-solar','Solar Finish','A burnished gold finish polished to a flare.',14,'#ffd24a','#8a6a12','#fff0b0','#ffe28a'),
 finish('finish-crimson','Crimson Finish','A blood red finish for close-range statements.',22,'#ff3b5c','#7f1d2d','#ffb3c0','#ff7a91'),
]);

export const CROSSHAIR_STYLES=Object.freeze([
 crosshair('cross','Crosshair','Classic four-line reticle for tracking targets.','cross',1,6,2,0),
 crosshair('dot','Dot','A minimal single-pixel dot for precision flicks.','dot',1,0,0,3),
 crosshair('ring','Ring','An open ring that frames the target without clutter.','ring',1,10,2,0),
 crosshair('chevron','Chevron','A downward chevron tuned for burst control.','chevron',8,8,2,0),
 crosshair('split','Split','A split reticle that reads gaps at long range.','split',16,12,2,1),
]);
