...
export class SoftwareRenderer{
 ...
 const color=(mat,light,alpha)=>{const c=mat.color.clone().convertLinearToSRGB();return `rgba(${Math.min(255,Math.round(c.r*255*light))},${Math.min(255,Math.round(c.g*255*light))},${Math.min(255,Math.round(c.b*255*light))},${alpha})`;};
 ...
 scene.traverseVisible(obj=>{if(!obj.geometry||!obj.material)return;const mat=Array.isArray(obj.material)?obj.material[0]:obj.material;if(!mat.visible||mat.opacity<.015)return;const pos=obj.geometry.attributes.position;if(!pos)return;matrix.multiplyMatrices(camera.matrixWorldInverse,obj.matrixWorld);const e=matrix.elements;let cached=this.cache.get(obj.geometry);if(!cached){cached={positions:Array.from(pos.array),indices:obj.geometry.index?Array.from(obj.geometry.index.array):Array.from({length:pos.count},(_,i)=>i)};this.cache.set(obj.geometry,cached);}const vertices=[];for(let i=0;i<cached.positions.length;i+=3){const x=cached.positions[i],y=cached.positions[i+1],z=cached.positions[i+2];vertices.push([e[0]*x+e[4]*y+e[8]*z+e[12],e[1]*x+e[5]*y+e[9]*z+e[13],e[2]*x+e[6]*y+e[10]*z+e[14]]);}
 if(obj.userData.label){const p=[e[12],e[13],e[14]];if(p[2]<-.1){draw.push({depth:-p[2],label:obj.userData.label,points:[project(p)],size:Math.max(5,obj.userData.labelSize*f/-p[2]),color:'#92d7cd'});}return;}
 const ix=cached.indices;if(obj.isLine||obj.isLineSegments){for(let i=0;i<ix.length-1;i+=obj.isLineSegments?2:1){const a=vertices[ix[i]],b=vertices[ix[i+1]];if(a[2]>=-.1||b[2]>=-.1)continue;const p=project(a),q=project(b);if((p[0]<0&&q[0]<0)||(p[0]>w&&q[0]>w)||(p[1]<0&&q[1]<0)||(p[1]>h&&q[1]>h))continue;draw.push({depth:-(a[2]+b[2])*.5,points:[p,q],color:color(mat,1,mat.opacity),line:true});}return;}
