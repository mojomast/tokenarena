...
test('every mode completes with combat or an objective result at every difficulty; 8-bot matches stay finite',()=>{for(const mode of GAME_MODES)for(const difficulty of DIFFICULTIES){const m=new Match('kimi','roo',rng(),resolveMapForMode('crosswire',mode.id,{legacy:true}),{mode:mode.id,difficulty:difficulty.id,botCount:8,timeLimit:60,fragLimit:5});for(let i=0;i<3601&&!m.over;i++)m.step(1/60);assert.ok(m.over,`${mode.id}/${difficulty.id}`);assert.ok(m.stats.shots>0,`${mode.id}/${difficulty.id} shots`);const settled=m.stats.kills>0||m.events.some(event=>['capture','zone-capture','assault-breach','payload-checkpoint','payload-delivered','objective-win'].includes(event.type));assert.ok(settled,`${mode.id}/${difficulty.id} resolves by combat or objective`);assert.ok(m.actors.every(a=>[a.x,a.y,a.z,a.health,a.frags].every(Number.isFinite)));}});
